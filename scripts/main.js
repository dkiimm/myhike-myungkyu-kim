...
function readQuote(day){
  db.collection("quotes").doc(day).onSnapshot(dayDocument =>{
    // console.log(dayDocument.data().quote)
    ...
    db.collection(collection).orderBy("length").get()   //the collection called "hikes"
        .then(allHikes=> {
            ...
            allHikes.forEach(doc => { //iterate thru each doc
                ...
                newcard.querySelector('i').onclick = () => updateBookmark(docID); //save bookmark when clicked
                newcard.querySelector('i').id = "saved-" + docID; //give unique id to each bookmark

                currentUser.get().then(userDoc =>{
                    let bookmarks_now = userDoc.data().bookmarks;
                    if(bookmarks_now.includes(docID)){
                        document.getElementById("saved-" + docID).innerText = "bookmark";
                    }
                })
